...
//This class illustrates the customer information stored in the database
 ...
 */
@Entity
@NamedQueries({ //finds all customers by name or email 
        @NamedQuery(name = Customer.FIND_ALL, query = "SELECT c FROM Customer c ORDER BY c.name ASC"),
        @NamedQuery(name = Customer.FIND_BY_EMAIL, query = "SELECT c FROM Customer c WHERE c.email = :email")
})
@XmlRootElement
@Table(name = "customer", uniqueConstraints = @UniqueConstraint(columnNames = "email")) //assigns email as unique key to the table
public class Customer implements Serializable {
    ...
    @NotNull //sets name constraints
    @Size(min = 1, max = 50)
    @Pattern(regexp = "[A-Za-z-' ]+", message = "Name should not include names or special characters")
    @Column(name = "name")
	private String name;
	
    @NotNull //sets mail constraints
    @NotEmpty
    @Email(message = "Mail format should be example@mail.com")
	private String email; //primary key
	
    @NotNull //UK phone number format
    @Pattern(regexp = "^[0]{1}[0-9]{10}$")
    @Column(name = "phone_number")
	private String phoneNumber;
    
    @JsonIgnore
    @OneToMany(mappedBy="customer", orphanRemoval=true) //when is removed applies cascade removal to the booking
    private Set<Booking> bookings = new HashSet<Booking>();
    
    public Customer() {}
    
    public Customer(Long id, String name, String email, String phoneNumber) { //customer constructor
    	this.id = id;
    	this.name = name;
    	this.email = email;
